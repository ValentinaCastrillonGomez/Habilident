...
export class FormatsService extends GenericService<Format> {
  ...
  private readonly inputConfig = new Subject<{ form: FormGroup<FieldsConfigForm>, rowType: RowType }>();

  get input$() {
    return this.inputConfig.asObservable();
  }

  setInput(form: FormGroup<FieldsConfigForm>, rowType: RowType): void {
    this.inputConfig.next({ form, rowType });
  }
}
